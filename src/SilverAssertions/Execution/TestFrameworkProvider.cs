...
internal class TestFrameworkProvider
{
    ...
    private static readonly Dictionary<string, ITestFramework> Frameworks = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["mstestv3"] = new MSTestFrameworkV3(),
        ["mstestv4"] = new MSTestFrameworkV4(),
        ["xunit2"] = new XUnit2TestFramework(),
        ["xunit3"] = new XUnit3TestFramework(), // Keep this the last one as it uses a try/catch approach
    };
    ...
    private ITestFramework AttemptToDetectUsingAppSetting()
    {
        ...
        if (!Frameworks.TryGetValue(frameworkName, out ITestFramework framework))
        {
            ...
            var message =
                $"SilverAssertions was configured to use the test framework '{frameworkName}' but this is not supported. " +
        ...
        if (!framework.IsAvailable)
        {
            ...
            var message =
                $"SilverAssertions was configured to use the test framework '{frameworkName}' but {innerMessage}. " +
                $"Please use one of the supported frameworks: {frameworks}.";
