...
namespace SilverAssertions.Formatting;
...
public class PredicateLambdaExpressionValueFormatter : IValueFormatter
{
    ...
    /// Instead of "(_.Text == value(SilverAssertions.Specs.Collections.GenericCollectionAssertionsSpecs+c__DisplayClass122_0).twoText) AndAlso (_.Number == 3)".
    /// </summary>
    private static Expression ReduceConstantSubExpressions(Expression expression)
    {
        ...
        //{
        //    // Also fallback if our expression rewrite is not supported.
        //    return expression;
        //}
    }
    ...
    private sealed class ConstantSubExpressionReductionVisitor : ExpressionVisitor
    {
        public override Expression Visit(Expression node)
        {
            ...
            if (!HasLiftedOperator(node) && ExpressionIsConstant(node))
            {
                ...
                {
                    // Some methods cannot be dynamically invoked; fall through to base.Visit
                }
            }
