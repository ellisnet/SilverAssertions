...
namespace SilverAssertions.Tests.Primitives;
...
public class ObjectAssertionTests
{
    ...
    public class BeOfType
    {
        ...
        //TODO: As of 1/25/2025, the following test is NOT throwing an exception; so the test is failing - I should understand why, and if this indicates that something is broken
        /*
        [Fact]
        public void
...
#pragma warning restore 436
            ...
            act.Should().Throw<XunitException>()
                .WithMessage(
                    "Expected type to be [TestAssemblyB.ClassC, SilverAssertions.Tests*], but found [TestAssemblyB.ClassC, TestAssemblyB*].");
        }
        */
        ...
        public void When_object_type_is_a_subclass_of_the_expected_type_it_should_fail()
        {
            ...
            // Assert
            act.Should().Throw<XunitException>().WithMessage(
                "Expected type to be SilverAssertions*DummyBaseClass, but found SilverAssertions*DummyImplementingClass.");
        }
    }
