...
using SilverAssertions.CallerIdentification;
...
public static class CallerIdentifier
{
    ...
    public static string DetermineCallerIdentity()
    {
        ...
        try
        {
            ...
            int lastUserStackFrameBeforeSilverAssertionsCodeIndex = Array.FindIndex(
                allStackFrames,
                startIndex: 0,
                count: searchStart + 1,
                frame => !IsCurrentAssembly(frame) && !IsDynamic(frame) && !IsDotNet(frame));

            for (int i = lastUserStackFrameBeforeSilverAssertionsCodeIndex; i < allStackFrames.Length; i++)
            {
    ...
    internal static bool OnlyOneSilverAssertionScopeOnCallStack()
    {
        var allStackFrames = GetFrames(new StackTrace());

        int firstNonSilverAssertionsStackFrameIndex = Array.FindIndex(
            allStackFrames,
            frame => !IsCurrentAssembly(frame));

        if (firstNonSilverAssertionsStackFrameIndex < 0)
        {
            return true;
        }

        int startOfSecondSilverAssertionsScopeStackFrameIndex = Array.FindIndex(
            allStackFrames,
            startIndex: firstNonSilverAssertionsStackFrameIndex + 1,
            frame => IsCurrentAssembly(frame));

        return startOfSecondSilverAssertionsScopeStackFrameIndex < 0;
    }
