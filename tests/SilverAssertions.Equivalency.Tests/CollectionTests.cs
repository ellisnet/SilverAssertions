...
#pragma warning restore format
    ...
    public void Comparing_lots_of_complex_objects_should_still_be_fast()
    {
        ...
        _output.WriteLine($"Currently running on Process Architecture: {processArchitecture}");
        var expectedSeconds = 20.Seconds();

        // RISC-V architecture is slower as of Feb 2026, so we need to allow more time
        if (processArchitecture.Contains("RiscV", StringComparison.OrdinalIgnoreCase))
        {
            expectedSeconds += 60.Seconds();
        }
...
#pragma warning restore CA1305
        ...
        // Assert
        act.ExecutionTime().Should().BeLessThan(expectedSeconds);
    }
