...
using static SilverAssertions.FluentActions;

namespace SilverAssertions.Tests.Specialized;

public static class TaskAssertionTests
{
    ...
    public class ThrowWithinAsync
    {
        ...
        public async Task When_task_is_canceled_before_timeout_it_succeeds()
        {
            ...
            taskFactory.SetCanceled(CancellationToken.None);
            timer.Complete();
    ...
    {
        [Fact] //[UIFact] //TODO: Decide if I still need this test to run using the Xunit.StaFact NuGet package
        public async Task When_task_completes_fast_it_should_succeed()
        {
            // Arrange
            var timer = new FakeClock();
        ...
        //TODO: As of 1/25/2025, this test is failing; the following (unexpected) exception is being thrown -
        //  Did not expect any exception, but found Xunit.Sdk.XunitException: Expected SynchronizationContext.Current not to be <null>.
        //  It seems likely that this is because it is not being run as a [UIFact] using the Xunit.StaFact NuGet package;
        //  so I might need to put back that dependency. More research is required.
        /*
        [Fact] //[UIFact] //TODO: Decide if I still need this test to run using the Xunit.StaFact NuGet package
        public async Task When_task_is_checking_synchronization_context_it_should_succeed()
        {
            ...
            {
                await Task.Delay(1);
                SynchronizationContext.Current.Should().NotBeNull();  //Currently failing, because it *is* null - when expected to .NotBeNull()
            }
        }
        */
    }
